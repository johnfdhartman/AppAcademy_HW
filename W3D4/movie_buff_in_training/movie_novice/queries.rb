...
def star_wars
  ...
  # hint: use 'select' and 'where'
  star_wars = Movie.where('title LIKE ?', '%Star Wars%')
  star_wars.select(:id, :title, :yr)
...
def below_average_years
  ...
  bad_movie_yrs = bad_movies.group(:yr)
  bad_movie_yrs.select(:yr, 'COUNT(*) AS bad_movies')
end
...
def alphabetized_actors
  ...
  actors = Actor.select(:id, :name).order(name: :asc).limit(10)
...
def pulp_fiction_actors
  ...
  Actor.joins(:castings).joins(:movies).where('title = ?', 'Pulp Fiction').distinct
...
def uma_movies
  ...
  Movie.joins(:castings).joins(:actors)
